    ...
    >
      ...
      <Box sx={{ mb: 1 }}>
        ...
        <Box display="flex" alignItems="center" gap={1} mt={0.5}>
          <Chip
            label={`${stats} total votes`}
            color={getChipColor()}
            size="small"
          />
          <Chip
            label={`${totalCount} unique ballots`}
            variant="outlined"
            size="small"
          />
        </Box>
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          ...
          {currentBallot?.count > 1 && (
            <Chip 
              label={`Count: ${currentBallot.count}`} 
              size="small" 
              color="secondary" 
              variant="outlined"
            />
              ...
              <Box>
                ...
                <Paper elevation={0} sx={{ p: 2, mb: 3, backgroundColor: 'grey.50' }}>
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={4}>
                      ...
                      <Typography variant="h5">
                        {Math.abs(ballotData.stats.cand1_wins - ballotData.stats.cand2_wins)}
                      </Typography>
                    </Grid>
                  </Grid>
                </Paper>
                ...
                {(() => {
                  ...
                  const getGridSize = (index, total) => {
                    ...
                    if (total === 2) return 6;
                    if (total === 3) {
                      // For 3 items, make them equal width
                      return 4;
                    }
                    if (total === 4) {
                  ...
                  return (
                    <Grid container spacing={2}>
                      {groupsToDisplay.map((group, index) => (
                        <Grid item xs={12} md={getGridSize(index, groupsToDisplay.length)} key={group.type}>
                          <BallotCarousel
